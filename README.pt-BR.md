...
| `--shader N` | Shader (`0`…`5`) |
| `--intensity F` | Intensidade do shader `0..1` (padrão `0.85`) |
| `--crt-preset N` | Preset CRT `0` Clean / `1` Realistic / `2` Broken |
...
| 4 | CRT | Multi-pass: curvatura, scanlines de feixe, máscara, bloom, falhas vintage |
| 5 | Soft Glow | Blur/glow |

**Opções do CRT** (`--intensity F`, `--crt-preset N`, ou Inspector → Display):

| Preset | N | Visual |
|--------|---|--------|
| Clean | 0 | Só ótica (curva, máscara, scan, bloom leve) — sem ruído/flicker |
| Realistic | 1 | Grain, flicker, barra rolante e wobble leves |
| Broken | 2 | Máximo vintage: static, flicker, roll e jitter fortes (padrão) |

Intensity (`0..1`, padrão `0.85`) escala todo o stack CRT.
